function [mat_path, dat_path] = BaselineAcqnvsPrairie_fb_playback(folder, animal, day, ...
    AComp, roi_mask, task_settings, a, playback_data, playback_bool)
 %{
...
%}
    ...
    global pl baseActivity playback
    ...
    playback        = zeros(1, expectedLengthExperiment) + nan;
    ...
    fwrite(fileID, playback,'double');
    fclose(fileID);
    % maps the file into memory
    m = memmapfile(dat_path, 'Format',...
        {'double',size(baseActivity),'baseAct'; ...
        'double',size(playback),'playback'}, 'repeat', 1); 
    m.Writable = true;
    %%
    %************************************************************************
    %*************************** RUN ********************************
    ...
    while counterSame < 500
        ...
        if ~isequal(Im,lastFrame)   
            ...
            if playback_bool
                ...
                %ASSIGN:
                playback(:,frame)           = fb_freq_i;
                m.Data.playback(:,frame)    = fb_freq_i; % 1 ms                
            end
...
function cleanMeUp(mat_path)
    ...
    save(mat_path, 'baseActivity', 'playback')
    if pl.Connected()
        pl.Disconnect();
    end
end
