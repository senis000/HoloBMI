...
%}
    ...
    num_var = size(data_mat, 1); 

    psth_len = win(2)-win(1)+1; 
    psth_mat = zeros(num_events, num_var, psth_len); 
    ...
    psth_mat(~event_valid, :, :)=[]; 

    %Average over events: 
    psth_mean = squeeze(mean(psth_mat, 1)); 
    psth_var = squeeze(var(psth_mat, 0, 1)); 
