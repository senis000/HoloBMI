...
%     fclose(fileID);
    ...
    data.frame = 1;
    nonBufferUpdateCounter = 40
    for sample_i=1:num_samples
        E1_i = f_data(bData.E1_base,data.frame); 
        E2_i = f_data(bData.E2_base,data.frame); 
        unitVals = [E1_i; E2_i]; 
...
%             m.Data.baseVector(:,data.frame) = baseval; % saving in memmap            
            ...
            [dff_z, cursor_i, target_hit, c1_bool, c2_val, c2_bool, c3_val, c3_bool] = ...                
...
disp('done')
cursor_BMI = data.cursor;
cursor_BMI(isnan(cursor_BMI)) = []; 
length(cursor_BMI)
%%
...
hold on;
hline(bData.T1)
title('cursor from BMI'); 

%%
prefix_win = 40;
f0_win = round(2*60 * frameRate);
dff_win = movingAverageFrames;
cursor_base = f2cursor_baseline(f_data, bData, prefix_win, f0_win, dff_win);
...
hold on;
hline(bData.T1)
title('cursor from baseline'); 
%%
prefix_win = 40;
f0_win = round(2*60 * frameRate);
dff_win = movingAverageFrames;
cursor_base_BMI = f2cursor_baseline_vBMI(f_data, bData, prefix_win, f0_win, dff_win);

h = figure; 
plot(cursor_base_BMI)
hold on;
hline(bData.T1)
title('cursor from baseline vBMI'); 
...
plot(cursor_BMI);  
plot(cursor_base_BMI); 
base_x = length(data.cursor)-length(cursor_base) + (1:length(cursor_base)); 
plot(base_x, cursor_base);
legend({'BMI', 'baseline', 'baseline_BMI'}); 

[rho, pval] = corr(cursor_BMI(:), cursor_base_BMI(:))

%%
sum(cursor_base >= bData.T1)
