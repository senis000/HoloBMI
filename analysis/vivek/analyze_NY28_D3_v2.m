...
%%
%ToDo: 
%make a better struct that has fields clearly: 
%f, f0, dff, dffz
...
win_zoom    = [25 30]; 
save_dir = plot_dir; 
% save_dir    = plot_dir; fullfile(plot_dir, 'U19'); 
% mkdir(save_dir); 
save_bool   = 1; 
plot_big    = 1; 
...
%%
block_data2event_mat_psth(); 
%takes the neural + cursor and returns the time series locked to event +
%psth
...
%%
block_data = pretrain_est; 
block_data.bData.E_id = [1 2]'; 
block_data.dff_z = block_data.dff_z([2 6], :);
...
%%
%Cursor Histogram: 
cursor = struct('base', [], 'pretrain', [], 'bmi', []); 
block_data = base_est; 
for i=1:length(block_data)
    cursor.base = [cursor.base block_data(i).data_valid.cursor]; 
end

block_data = pretrain_est; 
for i=1:length(block_data)
    cursor.pretrain = [cursor.pretrain block_data(i).data_valid.cursor]; 
end

block_data = bmi_est; 
for i=1:length(block_data)
    cursor.bmi = [cursor.bmi block_data(i).data_valid.cursor]; 
end
