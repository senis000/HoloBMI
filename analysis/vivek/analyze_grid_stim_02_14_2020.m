...
tif_dir = 'D:\DATA\grid_stim\200214\NVI20\D0\redgreen\stim-004'
data_dir = 'D:\DATA\grid_stim\200214\NVI20\D0\data_stim-004'
load_path = fullfile(data_dir, 'matlab.mat')
save_dir = 'D:\Dropbox\Data\grid_stim'
...
target_mask = zeros(dim, dim); 
for row = 1:dim
    for col = 1:dim
        dist = norm([col row] - [target_roi.x target_roi.y]); 
        if dist <= target_roi.r
...
%% 
im_bg_path = fullfile('D:\DATA\grid_stim\200214\NVI20\D0\data_stim-004', 'AVG_stim-004.tif'); 
exist(im_bg_path)
...
% %%
% % find(stim_sequence == ctr_idx)
% % stim_frame = find(stim_sequence == ctr_idx)
% target_stim_frame = stim_frame(find(stim_sequence == ctr_idx)); %%
% psth_win = [-300 300];
% num_samples_psth = psth_win(2)-psth_win(1)+1;
% stim_psth_mat = zeros(num_samples_psth, num_target_stims); 
% for i = 1:length(target_stim_frame)
%     win_i = target_stim_frame(i)+psth_win; 
%     stim_psth_mat(:, i) = target_f_ts(win_i(1):win_i(2));
% end
...
h = figure;
hold on;
for i =1:num_target_stims
    plot(x_plot, stim_psth_mat(:,i)); 
end
xlabel('time (s)'); 
ylabel('dff'); 
title('dff locked to target roi stim (locked at t=0)')
vline(0)

plot(x_plot, psth_mean, 'k', 'LineWidth', 1.3)

%%
%Plot mean + sem: 
y_plot = psth_mean; 
y_sem = psth_sem; %-min(y_plot); 
h = figure;
hold on;
...
% plot(x_plot, y_plot+y_sem, 'Color', 0.5*ones(1,3), 'LineWidth', 0.5)
% plot(x_plot, y_plot-y_sem, 'Color', 0.5*ones(1,3), 'LineWidth', 0.5)
% errbar(x_plot, y_plot,y_sem, 'Color', 'k'); 
...
pre_idxs    = find((x_plot >= -1 & x_plot < 0)); 
post_idxs   = find((x_plot >= 0 & x_plot <= 1));
...
% ylim([-0.1 1.1]); 
axis square
title('Percent change between 1 sec after stim vs 1 sec before stim'); 
%%
% h = figure; 
...
%     r_i = perc_change; 
    r = [r change_over_grid(grid_idx).perc_change]; 
end
...
ylabel('percent change in DFF, 1s before and after stim'); 
axis square
...
grid_and_target(:,:,1) = grid_and_target(:,:,1) + 0.5*roi_data.im_roi(:,:,3); 
...
scatter_size = 100; 
h = figure; 
imagesc(grid_roi_data.im_bg*1.4);
hold on; 
for grid_idx = 1:num_grid_pts
    val = change_over_grid(grid_idx).perc_change;
