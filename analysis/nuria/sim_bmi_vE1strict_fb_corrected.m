...
% CORRECTION!!! :  prefix_win = task_settings.prefix_win;
% in BMI:  nonBufferUpdateCounter = 40;  %counter when we dont want to update the buffer: 
% and initFrameBase = nonBufferUpdateCounter + 1; so...
prefix_win = 40;
...
if(dff_win_bool)
    ...
    %CORRECTION!!! this will average over the middle value, in the BMI
    % it used the last X values not the future ones 
...
%     end
    ...
    for i=1:num_samples
        f_smooth(i,:) = nanmean(f_raw(f0_win-dff_win+i:f0_win+i-1, :),1);
    end
...
if length(hit_idxs_no_b2base) > 1
    for i = 2:length(hit_idxs_no_b2base)
        %CORRECTION as it is it also counts the cursor at the moment of hit
        %b2base_bool = sum(cursor_obs(hit_idxs_no_b2base(i-1):hit_idxs_no_b2base(i)) <= b2base_thresh) >= back2BaseFramesThresh; 
        %change is absolutely irrelevant because by definition (being a
        %hit) will never be less than 0.5*T, but I still want to make it
        %proper. 
        b2base_bool = sum(cursor_obs(hit_idxs_no_b2base(i-1)+1:hit_idxs_no_b2base(i)-1) <= b2base_thresh) >= back2BaseFramesThresh; 
