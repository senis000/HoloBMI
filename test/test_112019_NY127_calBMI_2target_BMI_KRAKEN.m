%%
%test_110419_NVI12.m
code_dir = 'D:\Dropbox\Code'
data_dir = 'D:\Dropbox\Data'

%%
home_dir = fullfile(code_dir, 'holobmi_git', 'HoloBMI'); 
cd(home_dir); 
load_path = define_and_load_bmi_paths_debug(home_dir)
matlab_exchange_home = fullfile(code_dir, 'MatlabExchange'); 
addpath(genpath(matlab_exchange_home)); 

%%
animal          = 'NY127'
home_dir        = fullfile(data_dir, 'holo_nov2019'); 
...
%See the calibration reuslts: 
base_data = load(base_file); 
cal_data = load(cal_file)
cal_ALL_data = load(cal_ALL_file); 
bmi_data = load(bmi_file); 

disp('num E2 hits calibration:')
cal_ALL_data.E2_hit_cal.num_hits_b2base

disp('num E1 hits calibration:')
cal_ALL_data.E1_hit_cal.num_hits_b2base

E1_base         = cal_ALL_data.E1_base; 
E2_base         = cal_ALL_data.E2_base; 
%
%Constants:
frameRate = 29.989;
...
%Consider: 
%The tone distribution is not fixed from day to day... 
%Is this a problem? 
...
f0_bmi = bmi_data.data.baseVector;
f0_bmi = f0_bmi(idx_plot, ~isnan(f0_bmi(idx_plot,:))); 
plot(n_bmi(idx_plot,:)); 
plot(f0_bmi, 'linewidth', 3); 


%%
%Choose data to use for simulation: 
...
%--------------------------------------------------------------------------
%Simulate baseline through the BMI: 
%%
drop_prefix_bool = 0
sim_bmi_bool = 1
...
if drop_prefix_bool
    debug_input = debug_input(:,(prefix_win+1):end); 
end

disp('debug input size: ')
disp(size(debug_input))

[task_settings] = define_BMI_task_settings()
% task_settings_test = task_settings; 
% task_settings_test.f0_win = 100; 
...
%%
n_sel = 2; 
n_plot = n_bmi(n_sel,3601:end);
n_plot = n_plot - cal_data.cal.target.n_min = 
b_plot = bmi_data.data.cursor(~isnan(bmi_data.data.cursor)); %zscore();
...
%%
%Load results and compare to calibration: 
bmi_dir = fullfile(folder, animal, day); 
bmi_path = fullfile(bmi_dir, 'BMI_online191104T142626.mat'); 
% bmi_path = fullfile(bmi_dir, 'BMI_online191104T131729.mat'); 
exist(bmi_path)
...
% %Compare to actual BMI number of hits: 
